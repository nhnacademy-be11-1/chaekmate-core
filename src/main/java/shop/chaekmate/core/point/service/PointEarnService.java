...
public class PointEarnService {
    ...
    public CreatePointHistoryResponse earnPointForReview(Long memberId, Long reviewId) {
        log.info("[포인트 적립] 리뷰 작성 포인트 적립 시작 - 회원ID: {}, 리뷰ID: {}", memberId, reviewId);
        ...
        try {
            var policy = pointService.getPolicyByType(PointEarnedType.IMAGE_REVIEW);
            int reviewPoint = policy.point();

            if (reviewPoint <= 0) {
                log.info("[포인트 적립] 리뷰 작성 포인트가 0이므로 적립하지 않음 - 회원ID: {}, 리뷰ID: {}", memberId, reviewId);
                return null;
            }
            ...
            CreatePointHistoryRequest request = new CreatePointHistoryRequest(
                    ...
                    PointSpendType.EARN,
                    reviewPoint,
                    String.format("이미지 리뷰 작성 - 리뷰ID: %s", reviewId)
            );

            CreatePointHistoryResponse response = pointHistoryService.earnPointHistory(memberId, request);
            log.info("[포인트 적립] 리뷰 작성 포인트 적립 완료 - 회원ID: {}, 리뷰ID: {}, 포인트: {}", memberId, reviewId, reviewPoint);
            return response;
        } catch (shop.chaekmate.core.point.exception.PointPolicyNotFoundException e) {
            log.warn("[포인트 적립] 리뷰 작성 포인트 정책이 설정되지 않아 포인트를 적립하지 않음 - 회원ID: {}, 리뷰ID: {}", memberId, reviewId);
            return null;
        } catch (Exception e) {
            log.error("[포인트 적립] 리뷰 작성 포인트 적립 실패 - 회원ID: {}, 리뷰ID: {}, 오류: {}", memberId, reviewId, e.getMessage(), e);
            throw e;
        }
    }
}
