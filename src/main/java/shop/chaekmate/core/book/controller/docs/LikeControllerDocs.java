...
public interface LikeControllerDocs {
    ...
    ResponseEntity<LikeResponse> createLike(@Parameter(description = "좋아요를 추가할 책 ID", example = "1") @PathVariable Long bookId,
    ...
    ResponseEntity<LikeResponse> readLike(@Parameter(description = "조회할 좋아요 ID", example = "1") @PathVariable(name = "likeId") Long likeId);

    @Operation(summary = "책별 좋아요 목록 조회", description = "책 ID로 해당 책의 모든 좋아요를 조회합니다.")
    @ApiResponse(responseCode = "200", description = "조회 성공")
    ResponseEntity<List<LikeResponse>> getBookLikes(@Parameter(description = "조회할 책 ID", example = "1") @PathVariable Long bookId);

    @Operation(summary = "회원별 좋아요 목록 조회", description = "회원 ID로 해당 회원의 모든 좋아요를 조회합니다.")
    @ApiResponse(responseCode = "200", description = "조회 성공")
    ResponseEntity<List<LikeResponse>> getMemberLikes(@Parameter(description = "조회할 회원 ID", example = "1") @PathVariable Long memberId);

    @Operation(summary = "좋아요 ID로 삭제", description = "좋아요 ID로 좋아요를 삭제합니다.")
    @ApiResponse(responseCode = "204", description = "삭제 성공")
    @ApiResponse(responseCode = "404", description = "해당 ID의 좋아요를 찾을 수 없음")
    ResponseEntity<Void> deleteLikeById(@Parameter(description = "삭제할 좋아요 ID", example = "1") @PathVariable Long likeId);
    ...
    ResponseEntity<Void> deleteLikeByBookIdAndMemberId(@Parameter(description = "좋아요를 삭제할 책 ID", example = "1") @PathVariable Long bookId,
