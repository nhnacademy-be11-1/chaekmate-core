...
public interface LikeControllerDocs {
    ...
    @Operation(summary = "좋아요 생성", description = "책에 회원ID 로 좋아요를 추가합니다.")
    ...
    ResponseEntity<LikeResponse> createLike(
            ...
            @Parameter(description = "요청 사용자 ID", required = true, example = "1") @RequestHeader("X-Member-Id") String memberId);
    ...
    @Operation(summary = "책과 회원 ID로 삭제", description = "X-USER-ID 헤더에서 회원 ID를 추출하여 책 ID와 함께 좋아요를 삭제합니다.")
    @ApiResponse(responseCode = "204", description = "삭제 성공")
