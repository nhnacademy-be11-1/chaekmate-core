...
public interface CategoryControllerDocs {
    ...
    CreateCategoryResponse createCategory(@Valid @RequestBody CreateCategoryRequest createCategoryRequest);

    @Operation(summary = "카테고리 전체 조회", description = "모든 카테고리를 계층 구조로 조회합니다.")
    @ApiResponse(responseCode = "200", description = "조회 성공")
    List<ReadAllCategoriesResponse> readAllCategories();

    @Operation(summary = "카테고리 단건 조회", description = "ID로 카테고리를 조회합니다.")
    @ApiResponse(responseCode = "200", description = "조회 성공")
    @ApiResponse(responseCode = "404", description = "해당 ID의 카테고리를 찾을 수 없음")
    ReadCategoryResponse readCategory(@Parameter(description = "조회할 카테고리 ID", example = "1") @PathVariable(name = "id") Long categoryId);
    ...
    UpdateCategoryResponse updateCategory(@Parameter(description = "수정할 카테고리 ID", example = "1") @PathVariable(name = "id") Long categoryId,
                                        @Valid @RequestBody UpdateCategoryRequest updateCategoryRequest);

    @Operation(summary = "카테고리 삭제", description = "카테고리를 삭제합니다.")
    @ApiResponse(responseCode = "204", description = "삭제 성공")
    @ApiResponse(responseCode = "404", description = "해당 ID의 카테고리를 찾을 수 없음")
    void deleteCategory(@Parameter(description = "삭제할 카테고리 ID", example = "1") @PathVariable(name = "id") Long categoryId);
}
